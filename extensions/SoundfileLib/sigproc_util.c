...
vec64 create_vec64(int size)
{
    // create vector and set all elements to zero
    vec64 vector;
    // vector.size = size;
    //vector.data = (double*)fftwf_malloc(size*sizeof(double)); //gives you alinged memory
	vector.data = (double*)	(size*sizeof(double));
    //malloc_check(vector.data);	
    memset(vector.data, 0, sizeof(double)*size);
...
/*
float error_norm(vec32 x, vec32 y, float scale)
{
    int i;
    float s = 0;
    float diff;
...
*/
void free_vec32(vec32* x)
{
    if(x->data != NULL) //fftwf_free(x->data);
		free(x->data);
    x->data = NULL;
    //x->size = 0;
}

void free_vec64(vec64* x)
{
    if(x->data != NULL)
        //fftwf_free(x->data);
		free(x->data);
...
/*
void write_vec32(vec32 A, const char* file)
{
